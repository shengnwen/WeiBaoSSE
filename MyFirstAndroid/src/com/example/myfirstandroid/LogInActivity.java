...
public final class LogInActivity extends Activity {
    ...
    public void logIn(View view){
    	...
    	if(useraccount.getText().length()==0||userpassword.getText().length()==0){
    		Toast.makeText(getApplicationContext(),
				     "邮箱名和密码不能为空", Toast.LENGTH_LONG).show();
    	}
    	else{
	...
	class Verify extends AsyncTask
	{
        ...
        protected void onPostExecute(Object result){
			...
			if(Result == 1){
				...
				Toast.makeText(getApplication(), "账户名不存在!", Toast.LENGTH_LONG).show();
			}else{
				Toast.makeText(getApplication(), "登陆失败,请检查网络连接", Toast.LENGTH_LONG).show();
			}
